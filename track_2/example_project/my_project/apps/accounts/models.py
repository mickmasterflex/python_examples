...
class UserProfile(common_mixins.TimestampMixin, common_models.BaseModel):
    """
    The class stores extra info about the user.
    """
    ...
    # All the instances associated with this user
    instance = models.ManyToManyField(instance_models.Instance)

    def __unicode__(self):
        if self.user.first_name:
            if self.user.last_name:
                return '{0} {1}\'s Profile'.format(self.user.first_name,
                                                   self.user.last_name)
            else:
